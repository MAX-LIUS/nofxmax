...
func (at *AutoTrader) runCycle() error {
	...
	record := &store.DecisionRecord{
		...
		ReviewContext: map[string]interface{}{
			...
			"allow_ai_stop_close":  at.GetAllowAIStopClose(),
			"allow_ai_take_profit": at.GetAllowAITakeProfit(),
			"ai_stop_min_loss_pct": at.GetAIStopMinLossPct(),
	...
	if !at.GetAllowAIStopClose() {
		decisionVariant = decisionVariant + "|no_stop_close"
	}
	if !at.GetAllowAITakeProfit() {
		decisionVariant = decisionVariant + "|no_take_profit"
	}
...
// action is "close_long" or "close_short" — used to derive the position side.
func (at *AutoTrader) getCurrentPositionPnLPct(symbol, action string) float64 {
	side := "long"
	...
	for _, p := range positions {
		...
		return calculatePositionPnLPct(side, entryPrice, markPrice)
	}
	return 0
}
