...
type MechBucket struct {
	...
	PortfolioLvl bool    // trigger fires on portfolio-wide state (breadth/equity), not per-entry price
...
func MechanismFidelity(loaded []loadedEntry, baseline ProtectionParams) (buckets []MechBucket, trustedActual, trustedBt, dataLossActual, portfolioActual float64) {
	...
	for _, le := range loaded {
		...
		if b == nil {
			b = &MechBucket{
				Reason:        r,
				Confidence:    AttributionConfidence(le.entry.CloseReason),
				PortfolioLvl:  isPortfolioLevelReason(le.entry.CloseReason),
			}
			agg[r] = b
		}
		...
		switch {
		case b.PortfolioLvl:
			// Trusted attribution but portfolio-level trigger — not per-entry
			// price-replayable, so kept out of the price-fidelity metric.
			portfolioActual += le.entry.RealizedPnL
		case b.Confidence == AttrTrustedProtection:
	...
	return buckets, trustedActual, trustedBt, dataLossActual, portfolioActual
}
...
func FormatMechanismFidelity(loaded []loadedEntry, baseline ProtectionParams) string {
	buckets, trustedActual, trustedBt, dataLossActual, portfolioActual := MechanismFidelity(loaded, baseline)
	...
	var trustedN, lossN, portfolioN int
	for _, b := range buckets {
		sb.WriteString(fmt.Sprintf("%-24s %-6s %-5d %-11.2f %-11.2f %-10.2f\n",
			trunc(b.Reason, 24), confTag(b), b.N, b.ActualPnL, b.BtPnL, b.AbsErr))
		totAct += b.ActualPnL
		totBt += b.BtPnL
		switch {
		case b.PortfolioLvl:
	...
	// Fidelity is computed on the per-entry price-replayable trusted subset only.
	trustedRelErr := 0.0
	if trustedActual != 0 {
		trustedRelErr = (trustedBt - trustedActual) / absf(trustedActual) * 100
	}
	sb.WriteString(fmt.Sprintf(
		"\n[trusted price-replayable]  N=%d  actual=%.2f  replay=%.2f  rel_err=%.1f%%\n",
		trustedN, trustedActual, trustedBt, trustedRelErr))
	sb.WriteString(fmt.Sprintf(
		"[portfolio-level trusted]   N=%d  actual=%.2f  (breadth/equity trigger — not per-entry price-replayable, excluded from fidelity)\n",
		portfolioN, portfolioActual))
	sb.WriteString(fmt.Sprintf(
	...
	sb.WriteString("  only the reason label is untrusted. Fidelity is judged on per-entry TRUST rows only.\n")
	return sb.String()
}

// confTag renders a bucket's tier tag, distinguishing portfolio-level trusted
// rows (PORT) from per-entry trusted rows (TRUST).
func confTag(b MechBucket) string {
	if b.PortfolioLvl {
		return "PORT"
	}
	return confShort(b.Confidence)
}
