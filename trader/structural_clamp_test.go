...
func TestClampStructuralBoundary(t *testing.T) {
	// FallbackATRMul defaults to 2.5 via WithDefaults; beyond-backstop now falls back
	// to the tighter fallback (2.5 ATR), not the wide backstop (4.5 ATR).
	ss := store.StructuralSLConfig{Enabled: true, FloorATRMul: 1.5, BackstopATRMul: 4.5, LookbackBars: 24}
	atr := 100.0
	entry := 10000.0

	// LONG, raw swing 6.2 ATR away (beyond backstop 4.5) -> fall back to 2.5 ATR below entry.
	if got, ok := clampStructuralBoundary(entry, entry-620, atr, 0, true, ss); !ok || math.Abs(got-(entry-250)) > 1e-6 {
		t.Fatalf("long beyond-backstop: got %.2f ok=%v want %.2f", got, ok, entry-250)
	}
	...
	// SHORT, raw swing 6.2 ATR above -> fall back to 2.5 ATR above entry.
	if got, ok := clampStructuralBoundary(entry, entry+620, atr, 0, false, ss); !ok || math.Abs(got-(entry+250)) > 1e-6 {
		t.Fatalf("short beyond-backstop: got %.2f ok=%v want %.2f", got, ok, entry+250)
	}
