...
> **状态**: 生产运行 | **线上 = v1.16.10(2026-07-27 04:12 UTC pid 2274258 md5 9fd1402d,回滚备份 `/opt/webstack/nofx/nofx.bak_v1169_20260727_041214`)** | **v1.16.11 已编码待确认部署(md5 478e8ed8:ATR 档位分配单位错乱 → 部分止盈档按整仓挂单)**
...
> **版本**: v1.16.11 (待部署:档位分配 ATR→% + 身份匹配) / v1.16.10 (已部署:兜底匹配排除兄弟档已认领单) / v1.16.9 (已部署:开仓 ATR 换算 + entry 校正) / v1.16.8 (已部署) / v1.16.7 (三条 arm 分支补落库) / v1.16.6 (同 ruleFP 记录去重) / v1.16.5 (collapse 保留兄弟档) / v1.16.4 (取最新 arm 记录) / v1.16.3 (全档 cooldown 兜底) / v1.16.2 (orderID 身份,引入全档 churn) / v1.16.1 / v1.16.0 (近价锚定,已弃) / v1.15.0

> **🔥 v1.16.11 ATR 策略下档位分配存的是裸 ATR 倍数 → 30% 部分止盈档按整仓挂单(2026-07-27,该 bug 类第 8 次实例,已编码待部署 md5 478e8ed8)**
>
