...
> **版本**: v1.17.7 (**已部署 2026-08-02 14:04 CST pid 626448 md5 5acea85ffeac6fd18c16c5503a085b91,提交 `6a817f2`,回滚备份 `/opt/webstack/nofx/nofx.bak-v11705-20260802-060338` = md5 69a26400 / commit `9a48070`**;核对 health 200/单进程/NRestarts=0/WARN 0/panic 0/**撤单 0 挂单 0 = 零 churn**/4 持仓全 verified=true staleTrail=0;唯一 ERRO `drawdown_claim_conflict.go:300` 是**既存**问题(重启前同日已 9679 条,频率一致);**⚠️ 主修复点无实盘证据** —— `status=activated` 只在已武装 DD 档上打印,重启后 4 持仓全 `arm pending`,核心判据目前只有单测背书,下次武装时必须回看:追查"交易所 DD 为什么被重新挂"挖出两件独立的事 —— ①OKX 51149「Order timed out」是**半成功**(单子落地、响应体丢失),我们当硬失败丢弃且**没有按 `algoClOrdId` 回读**,于是所有权账本无 orderID、无法认领 → 10 秒后补挂;②OKX 适配器的 trailing 激活状态是从**哪个查询桶**回的行推断的,而不是读行自己的 `state`/`moveTriggerPx` → `pending_activation` 4 天 1528 次 vs `activated` 5 次,叠加 `nativeTrailingEffective` 对健康单逻辑反转,把在场活单判成"present but NOT effective" → 熔断 → `*_exchange_failed_armed` 是**单向锁**,`drawdown_order_reclaim.go` 零处 `setProtectionState` → ETHUSDT short 面板假报 ~410 次。**只改判据与回读,不撤任何交易所挂单**) / v1.17.6 (**待部署**:全量日志合规审计(64 笔平仓全合规)顺带挖出 4 个缺陷 —— ①BE 两档抢同一张单(0.1503% 间距落在 0.2% 匹配容差内 + reconciler/monitor 两条规则管线不同源,账本翻转 53 次);②reclaim 分支裸 return 导致 7688 条 "not verified" 假警告、BTCUSDT LONG 连续 1900 轮假 degraded(保护其实在位),顺带把容忍/reclaim 两处内联布尔式收口成唯一判据 `protectionCoverageComplete`;③time-stop/max-hold 在 NO_POSITION 上打 ✅(纯可观测性,幂等未坏,但按日志计数会翻倍);④OKX 51277 触发价已被市价越过的 TP 档被计入 tier 失败 → 整份计划连坐,其余 3 档 TP+SL 其实都挂上了。**不撤任何交易所挂单,不改变实物挂单行为**) / v1.17.5 (**已部署 2026-07-30 13:34 pid 164364 md5 abca7a86df493492d5082b67209980a4,提交 `4918a8d`,回滚备份 `/opt/webstack/nofx/nofx.bak-v11744-20260730-053404` = v1.17.4b md5 e7ce2f20**:核对 health 200/单进程/NRestarts=0/ERROR+panic 0/部署后亏损中棘轮 0 次/XAU 已有边界 4039.49 未被放松;`staleTrail=1` 在 ETH/BTC/SOL 上是**既存**问题(今日 1729 次、绝大多数在部署前,同四个币重启前后一致,SPCX 反而自愈),表现为 reclaim 41 次 / "nothing to cancel" 41 次**完全配平、零撤零挂**=无 churn,但账本改写不落地导致每轮重认领、面板显示 degraded 而保护其实在位 —— 与本次改动无关,**待单独处理**:棘轮结构位默认值订正 —— `TrailMinProfitATR` 指针化 nil→1.0(显式 0 仍关闭)、`TrailOnLoss` 默认 false、回测手抄默认值收口到 store 访问器) / v1.17.2 (**待部署**:v1.17.1 的档位标识在 ATR 单位策略上恒返回 0 = 死代码,4 个实盘交易员全中;reclaim 是 `getActiveDrawdownRulesForPosition` 唯一漏 `resolveDrawdownRulesATR` 的消费者 → BN WLDUSDT 4 条 armed 记录抢 2 张单且映射反向;新增"一单一主"不变量,判据用交易所形状而**不是** UpdatedAt —— 线上错的那条恰恰更新) / v1.17.1 (**已部署 `90de970`,但档位标识在 ATR 策略上从未生效,见 v1.17.2** 提交 `8133f80`:挂单 clientOrderID 自带档位标识 `<prefix><code2>T<1-9><nonce>` 作多档 DD 第二重身份,补 reclaim 把 dd2 单认给 dd1 的错认) / v1.17.0 (**已部署 2026-07-29 pid 2696350**:DD 不在交易所 + managed 陪跑形同虚设,6 根因 RC-1…RC-6 一次性收口 —— 假成交按归属判定/managed 不再压制补挂/熔断 30min 冷却衰减/活单拒撤改重写账本/覆盖判定要活单背书/武装门禁改 max(cur,peak);另注意 Go 文件名 `_arm` 会被当 GOARCH 静默跳过整个测试文件) / v1.16.28 (**已随 v1.17.0 部署**:加仓后保护单数量不跟涨 → 静态 SL/TP/兜底 + BE 逐档双路径的**量维度**收敛,只撤不挂/只管不足不管超出/量化后再比,两条路径用 BE 认领集合互斥防抢单;Binance 端核实 GetOpenOrders 两条命名空间都回 Quantity、CancelAlgoOrderByID 带 -2011 回退能撤两种单 → resize 在 BN 全链路可用,cTime=0 不影响) / v1.16.27 (休眠风险专项:能力表位置初始化→键名、venue callbackRate 能力从注释变数据、空 orderID 记录在不能模糊匹配的交易所上判为下单失败落本地 monitor,提交 `92a17f6`) / v1.16.26 (已部署:在场档位比例锚定当前量,ZEC ping-pong 根因,提交 `b292774`) / v1.16.25 (已部署 2026-07-28 13:38 pid 2479237 md5 1b5a608a:所有权日志分歧+黄灯 unknown_mark+runner 迁移撤错档,提交 `7efc6cf`) / v1.16.24 (待部署:挂单等价性按交易所粒度判定 + 阶梯档被推断执行后免撤,提交 `9682f65`) / v1.16.23 (已部署:前端归因枚举补齐 5 处 + 契约测试锁住前后端对齐,提交 `f3ee04e`) / v1.16.22 (已部署:保护档按成交价排序 + callback 单位收口 + 细粒度归因保留 + 开仓竞态宽限期,提交 `de6a3a5`) / v1.16.18 (待部署:OKX tag 装不下 reason → 定向撤单实为广撤;reason 收口到 coded client id) / v1.16.17 (待部署:trailing 归属按认领集合判定 + 档位分配锚定开仓量,提交 `87490ba`) / v1.16.16 (已部署:ATR→% 换算的除数锚定到冻结开仓价,提交 `80845a2`) / v1.16.15 (已部署:梯度身份与开仓均价解耦 + drawdownState 一格两用拆分,提交 `75854df`) / v1.16.14 (已部署:immediate trailing 落库归属 + 撤单点按返回值收口,提交 `b4f35e0`) / v1.16.13 (已部署:managed 全程陪跑双保险,取消账户级接管,提交 `b18ff54`) / v1.16.12 (待部署:全平档不占部分档预算 + supersede/累加/规则匹配四处配套) / v1.16.11 (已部署:档位分配 ATR→% + 身份匹配) / v1.16.10 (已部署:兜底匹配排除兄弟档已认领单) / v1.16.9 (已部署:开仓 ATR 换算 + entry 校正) / v1.16.8 (已部署) / v1.16.7 (三条 arm 分支补落库) / v1.16.6 (同 ruleFP 记录去重) / v1.16.5 (collapse 保留兄弟档) / v1.16.4 (取最新 arm 记录) / v1.16.3 (全档 cooldown 兜底) / v1.16.2 (orderID 身份,引入全档 churn) / v1.16.1 / v1.16.0 (近价锚定,已弃) / v1.15.0

> **🔥🔥🔥 `protectionState` 一格装五件事 —— SOLUSDT 死锁的根因是数据结构,不是逻辑(2026-08-04,已实现待部署)**
>
> **起因**:用户看完"武装记忆被擦除"的白名单式修复后追问「如何完美修复问题,好好研究,这看起来不是棘手问题」。确实不棘手 —— 它是个数据结构问题。
>
> **根因**:`protectionState map[string]string` 每个仓位**只有一格**,却同时承载五件互不相干的事:
> ①动态保护武装进度(native/managed × arming/armed × 全量/部分,**8 个值**);②交易所静态保护校验通过;③对账失败;④回撤已触发并平过仓;⑤面板展示用的最后观测。
> **写 ②~⑤ 的人会顺手擦掉 ①**。SOLUSDT 死锁全链条:挂单被 OKX `51279` 拒 → 写 `reconcile_failed:` → 覆盖 `native_trailing_armed` → 内存说没武装 / 账本说已武装 → 438 轮 degraded + 433 次 reclaim,**谁都不会去修对方**。
> 代码里其实早就写着这句话,`auto_trader_risk.go:3223`:*"protectionState is a single string per position that cannot express 'both tiers armed'"* —— 那条注释是在解释另一个 bug(SPCX 全平档武装时把部分档的活单撤了),同一个根因的另一个出口。
>
> **为什么白名单式修法必然反复漏**:之前(含我上一轮)都是在每个覆盖点加"前值是武装态就别写"的判断。**漏点数量 = 写入点数量(20 个)**,任何人新增一个写入点就重新漏一次,而且漏的时候**没有任何编译期或测试期信号**。那份手写并集历史上真的漏过 `managed_drawdown_armed`(见 `isManagedDrawdownProtectionState` 的注释),表现为交易所校验通过那一轮把 managed 武装记忆冲掉、下一轮又重新武装。
>
> **修法 —— 按维度分家,不变量钉在存取器上**(新增 `trader/protection_state_dimensions.go`):
> - 武装进度存 `protectionState`,非武装观测存新的 `protectionObservation`,**同一把锁**。
> - `setProtectionState` 仍收字符串(**20 个调用点、83 处测试断言全不用改**),但先用 `classifyProtectionStateWrite` 判归属,再**只写那一个维度**。于是"写观测擦掉武装"从**需要每处提防**变成**结构上不可能**;归属判定只有一处,新增状态值只改那张表。
> - `getProtectionState` 仍返回单一字符串(**武装优先、其次观测**),所有读者与谓词零改动。
> - `drawdown_triggered*` 是唯一"观测且必须同时清武装"的语义(`protectionDimObservationResetsArm`):managed 已平过仓,仓位数量变了,原武装记忆指向的指纹不再成立,留着会骗过重新武装的门禁。
> - **删掉两个已成冗余的白名单**:`protectionStateAfterReconcileFailure` 整个函数、reconciler `ExchangeVerified` 分支的 `isDynamicDrawdownArmState` 跳过写入。留着等于同一条不变量有两套机制,**后人无法判断哪个才是承重的**。
>
> **⚠️ 一个反直觉但必要的细节(差点自己引入更严重的 bug)**:`claimProtectionArmingState` 的 CAS 比较基准**必须是合成值** `deriveProtectionState`。调用方传进来的 `expectedCurrentState` 来自 `getProtectionState`(合成值),若这里只读武装维度,那么"武装维度为空、观测是 `exchange_protection_verified`"的仓位(**线上最常见形态**)会永远 CAS 不成立 —— **从未武装过的仓位再也武装不上**,比原 bug 更严重。已单独立测试钉住。同理回滚只能还原**武装维度**(`rollbackProtectionArmingState`):arming 期间 reconciler 可能已往观测维度写了新值,原先的"整格覆盖"会把它抹掉,还会把观测值当武装值写回去。
>
> **门禁**:`go build ./...` + `go vet ./trader/...` 干净;全仓 `go test ./...` 全绿(`nofx/trader` 44.9s);`-race` 跑保护相关全部用例无 data race;**11 个新用例逐个破坏性验证** —— 退回整格覆盖→4 红、CAS 只比武装维度→2 红、回滚连观测一起抹→1 红、`clear` 忘清观测维度→1 红,注入痕迹已确认清零。其中 **2 个用例在注入下不失败是故意的**:它们防的是**过度修正**(观测必须保持可见,`giveback_guard` 的 `state == "exchange_protection_verified"` 判断依赖这一点)。
>
> **⚠️ 本次没动的既有风险**:`auto_trader.go:658-666` 启动恢复**只读账本不核对交易所**,陈旧 armed 记录会掩蔽 `missingTP`。与本次分家是两件事,待单独处理。
> **⚠️ 尚未解决的同根问题**:武装维度仍是**单值**,表达不了"全平档与部分档同时武装"。`auto_trader_risk.go:3223` 那段"只撤孤儿单"的补丁仍是当前唯一防线。彻底解法是武装维度改成"全量/部分两个独立槽",属于下一步。
...
>
> **起因是用户问「近两天棘轮锁紧次数/币种/止损效果/后续走势」,不是报 bug**。查下来才发现默认值撒谎。
>
> **两个默认值合起来造成的**(`store/strategy.go`):
> - `TrailMinProfitATR` 是 `float64` + `omitempty` → **"未设置"与"显式 0"不可区分**。
>   5 个策略全部未设置 → JSON 无该键 → 归一化读回 **0 = 闸门关闭**。
>   而字段注释写 "Default 1.0"、UI 渲染 `?? 1.0` —— **文档和界面都在说谎**。
> - `TrailOnLoss` 默认 `true` → 亏损中也锁紧。
>
> **实盘后果**:ETHUSDT LONG 00:23 开仓、**00:24 就棘轮**,触发价 1921.15 对入场 1921.21,
> 根本没进入盈利。近两天 8 次锁紧里 6 次发生在亏损中。
>
> **危害的准确表述(我第一版判断错了,必须记住怎么错的)**:
> 我先按近两天 6 笔样本想给"棘轮"整体定性为有害,**站不住**:
> - `structural_sl` 是**天生 0 胜率**的出场口径 —— 全历史 97/97 全亏,其中 **89 笔根本没棘轮**。
>   所以"棘轮出场都是亏的"不能算棘轮的罪。
> - 每笔亏损幅度上棘轮其实更**轻**:棘轮组 -1.68%,静态结构位组 -2.01%。
> - 控制持仓时长(≥3h)横比:棘轮组胜率 63%/均值 +0.40%,无棘轮组 57%/-0.04%。
> - 置换检验「亏损中棘轮 vs 同期对照」差 -0.22pp,**p=0.312,不显著**。
>
> **真正站得住的判据是逻辑而非统计**:亏损中锁紧**在定义上锁不住任何利润**,
> 它唯一的作用是把失效判定从"结构破了"挪进噪音带 —— 出场原因从"论点失效"变成"噪音"。
> 因果证据(不是相关):3 笔有 K 线覆盖的亏损中棘轮,**不棘轮时的入场结构位全部从未被击穿**,
> 即这 3 次止损**只因棘轮而存在**;止损后价格分别反向走出 +2.36% / +2.48% / +4.72%,实亏合计 -9.31 USDT。
> **最干净的天然对照**:两笔 WLDUSDT SHORT 相隔 **2 分钟**开仓(0.3049 / 0.3036),
> 收紧 2.01% 的那笔被结构位打掉 **-3.40**;收紧仅 0.15% 的那笔活过同一根冲高,
> 21:27 以 **+0.47** 保本出场。同币同向同时段,唯一差别是棘轮收紧幅度。
>
> **修法**:`TrailMinProfitATR` 改 `*float64`(nil→1.0,**显式 0 仍表示关闭闸门** ——
> UI 的 `min="0"` 让 0 是用户可达的合法值,所以不能用负数当哨兵,否则界面上不可见);
> `TrailOnLoss` 默认改 `false`;新增 `TrailMinProfitATRValue()` 访问器。
> **不需要写生产库** —— 5 个策略两个字段全部未设置,改代码默认值即生效(归一化结果从不回写 DB,
> 每个读取点每轮重新 `WithDefaults()`,已验证幂等)。
>
> **顺带挖出的漂移**:`trader/backtest/config_baseline.go` **手抄**了一份 nil 默认判断
> (`ss.TrailOnLoss == nil || *ss.TrailOnLoss`),两边各写一份 → 线上默认改了回测不会跟,
> 回测会继续在亏损中锁紧。已改为调用 store 访问器。**手抄默认值就是漂移的来源。**
>
> **反向验证**:把 4 处默认值改回旧值 → `store` 6 例测试转红并打印
> "unset min-profit gate must default to 1.0 ATR, got 0"。
> **回放验证**:把近两天 8 次真实事件喂进新闸门 → 拦截 7 次、放行 1 次
> (放行的是 SKHYNIX 顺向 2.68ATR,实际 +2.37 盈利出场);3 笔 -9.31 USDT 的
> structural_sl 亏损全部被拦。XAU 那次顺向 0.96ATR **差 0.04 被拦**(边缘案例,当时在盈利中)。
> 已持仓的 XAU 边界 4039.49 保持不变 —— 棘轮只收紧不放松,新闸门只管未来的锁紧。

> **🔥🔥🔥 v1.17.3 被账本"屏蔽"的委托泄漏 —— 危害在交易所侧,而账本侧看起来完全干净(2026-07-29,提交 `17e0f88` + `1edc4f6`,已提交未部署)**
>
> **现象**:线上 ETHUSDT 空头同时挂着 **4 张 trailing,其中 3 张都是全平**,回调率各不相同。
> 而协调器**永远报 `staleTrail=0`** —— 它从来没觉得有问题。
>
> **危害必须说清楚,别被"账本干净"骗了**:账本侧确实自洽,坏在交易所侧 ——
> 谁先触发谁就平掉**整个仓位**,而泄漏出来那张回调**最紧**的单会比任何在用档位
> 设计的时机都更早打掉仓位。用户看到的是"莫名其妙被提前平了",查账本查不出东西。
>
> **根因(v1.16.15 那次修复留下的尾巴)**:`drawdownRuleIdentity` 把 entry/qty 抹成 `*`,
> 但**保留 ATR 解析后的 min/max 百分比**。ATR 重新冻结、或倍数被人调整时
> (此处 0.9 → 1.0 → 1.5 ATR),**同一个档位会算出不同身份**,旧记录就留在账本里继续 armed。
> 接着是关键的一环:`classifyProtectionOrder` 只要 `trailingOwnership` 认得该 orderID
> 就标记 `expectedDynamicOwner` —— 于是**这些泄漏记录反过来"屏蔽"了它们对应的委托**,
> 让 stale 清理逻辑一辈子看不见它们。泄漏的账本记录成了泄漏委托的**保护伞**。
>
> **修法用基数不变量,不要用比例之和**:我第一版写的是"认领比例加起来超过 100% 就收敛",
> **被我自己的测试打回**(`claimed exposure still 130.0%`)—— 100%(全平)+30%(部分)
> **本来就是正常两档配置**。正确判据是基数:已配置档位数 < 活跃且被认领的 trailing 单数时,
> 逐张回看"还有没有哪个**当前**档位认这张单",没有的把账本认领标记 `superseded`,
> 交回既有 stale 清理路径。两道基数闸门,取证不足一律不动手;
> 全部都能对上时**不猜**,报 ERROR 并保留所有认领。
> 顺序放在 `resolveConflictingTrailingClaims` **之后** —— 先收敛"一张单多档",
> 敞口计数才不会把同一张单重复计算。
>
> **反向验证(这条最值得记)**:把两道基数闸门删掉,**测试全绿** —— 说明闸门本身没有覆盖。
> 补 `ATRReresolutionWindowMustNotUnshieldTheOnlyLiveOrder` 后确认转红
> (`retired 1 records during an ATR re-resolution window`)。9 个测试用线上真实数值(入场 1890.56)。
>
> **顺带补掉的观测缺口**(`1edc4f6`):档位标识 `T<N>` 只写在 clOrdId/clientAlgoId 里,
> 放单后日志不打印,线上要核对"某张委托属于哪个档"只能去交易所查。OKX 与 Binance
> 成功日志各补一个字段。**订正我自己上一轮的部署记录**:那里写"冲突按交易所形状解决",
> 但日志普查显示实际判据是"是否属于该仓位当前档位集合",不是形状。

> **🔥🔥 v1.17.2 v1.17.1 的档位标识在 ATR 单位策略上是死代码,以及它撑出的"一单多主"账本腐烂(2026-07-29,4 处修复)**
>
> 用户裁决:"同意修复所有问题"。这一轮的性质与 v1.17.x 前面几次不同 —— 不是"新功能有 bug",而是**上一轮刚部署的新功能在 4 个实盘交易员上一次都没生效过**,而且它的缺位还让另一个更深的账本缺陷得以持续。
>
> **D-1 `drawdownTierTagIndex` 恒返回 0(v1.17.1 整个第二重保险是死代码)**。它拿传入的 rule 去比 `at.config.StrategyConfig.Protection.DrawdownTakeProfit.Rules`,判据是**精确浮点相等**。但 arm 路径拿到的 rule 已经过 `resolveDrawdownRulesATR` 解析(`min_profit_pct: 2.5, unit: atr` → `3.6791`),config 里存的还是原始倍数 `2.5` —— 永远不可能相等。5 个策略里 4 个(BN/Claude15/GPT-ct50/claude-ct30)全是 ATR 单位,只有 Claude-Orig 是 pct,所以**部署当天线上就没有一张单带上过 `T<N>`**,日志里全是裸 `reason="native_trailing"`。
> - 修法:档位序号的定义从"在原始配置里的位次"改成"在**该仓位当前生效的、已 ATR 解析的**规则集里的位次",新增 `canonicalDrawdownTierRules(symbol, side, entryPrice)` 作**唯一基准**,挂单侧与认领侧都必须经由它。顺带修好两件事:AI 规则(`drawdownAIRules`)以前根本不在 `config.Rules` 里、序号必然 0,现在也能带标识;三键规范排序(MinProfitPct → MaxDrawdownPct → CloseRatioPct)避免同 MinProfitPct 两档的位次在两次调用间漂移。
> - 浮点相等换成 `1e-9` 个百分点的绝对容差:两侧的百分比都由 `EffectivePercent` 现算,输入相同但运算顺序不同就会差 1 个 ULP,精确相等会因此判成"不是同一档",又回到序号恒 0。1e-9 远小于任何真实档距(实盘最近两档差 2.2 个百分点)。
>
> **D-2 `reclaimLiveDrawdownTrailingOrders` 用未解析的规则算 activation/callback/fingerprint(这才是 4 条记录抢 2 张单的根因)**。它是 `getActiveDrawdownRulesForPosition` 的**唯一**没有跟着调 `resolveDrawdownRulesATR` 的消费者(risk.go:210 / decision.go:1270 / reconciler.go:570 / execution.go:373 全都有)。于是把 "2.5 个 ATR" 当 "2.5%" 算,BN WLDUSDT short(entry 0.3036, ATR(1h)=0.004616 → 1.5204%/ATR)上:
>
>       未解析算出 activation 0.296010 / 0.291456   ← 落库的 armed 记录
>       交易所真单 activation 0.292060 / 0.285137   ← 按解析后的 3.6791%/5.8865% 挂的
>
>   形状匹配的 1% 容差刚好卡不住(漂 1.33%),于是认领走不了形状匹配、退到 "activated 短路 + 取最低未用档",把两张单**认反了**;更糟的是落库的 fingerprint 带 `min=2.5000`,与 arm 侧的 `min=3.6791` 在 `drawdownRuleIdentity` 看来"不是同一档",`supersedeOlderArmedRecords` 的窄匹配(同档 + 不同 orderID + 更旧)**永远碰不到它们** → 4 条 armed 记录抢 2 张实物单,每 ~20s 重写一次。
>
> **D-3 账本缺一条不变量:一张活单只能有一条 armed 记录认领它**。危害不是"多一条记录":`storedTrailingOrderIDForRule` 按 `UpdatedAt` 取最新,而 30% 那一档一旦成交,100% 那一档可能把自己标成已执行 → **全平回撤保护静默消失**。新增 `trader/drawdown_claim_conflict.go`,在 reconcile 每轮(手里已有 openOrders,不发额外请求)收敛跨档冲突。
> - **这里踩了一个我自己的坑,值得单独记**:我第一版把清扫放在启动加载(`loadDynamicProtectionStateFromStore`),判据是"保留 UpdatedAt 最新的一条"。随后去查线上真实账本,发现**错的那两条恰恰更新**(upd 1785334682 vs 1785326661)—— 因为坏掉的 reclaim 每 20s 重写它们。**按"取最新"清扫会留下错的那条,比不清扫更糟**。已整段删掉重做:判据改成三级,① 记录描述的形状是否对得上交易所那张真单(交易所是唯一事实来源)② 档位是否还在该仓位当前生效的规则集里 ③ 都分不出来才按时序,并打 WARN 明说"这次是按时序猜的"。放在 reconcile 而不是启动,就是因为启动时手里没有交易所数据、唯一可用的判据恰好是错的那个。
> - 已激活的单不参与形状判定(激活后 StopPrice 是移动跟踪价、Binance 干脆不报 callback,拿它比对计划值必然失配,会把**正确**的记录判成错的),弃权后落到第二级判据。
> - 只处理**跨档**冲突;同档的陈旧分叉仍归 `supersedeOlderArmedRecords` 的时序逻辑,不越权(有反向锁测试)。
>
> **D-4 `tierTaggedRuleIndex` 曾对每条规则反查一遍序号**。改成直接用序号当下标之后,多了一个新暴露面:序号是**挂单当时**编进 clientOrderID 的,而规则集是**此刻**的,中间可能重排(人工改策略;或混合单位配置下 ATR 解析只缩放其中一档而把先后倒过来 —— 当前 5 个策略都是纯 ATR 或纯 pct,同比例缩放不改先后,但代码不能只对当前配置成立)。加了一道"能查就查"的形状交叉验证:单子还没激活且报了可用 StopPrice 时,形状必须仍然像它自报的那一档,不像就放弃标识、退回形状匹配。这一步只可能让结果更保守(改动之前根本没有标识,一律走形状匹配),却挡住了"认错档"这个唯一危险的失败模式。
>
> **测试 17 例,4 处修复逐个反向验证(还原任一处对应用例即刻转红)**,含 7 例破坏性 + 6 条反向锁:冻结 ATR 缺失时两侧同基准仍自洽 / 开仓均价漂移的**真实契约**(同 entryPrice 严格幂等;漂出容差且无 cTime 时只能退化为 0、绝不能撞上别的档;cTime 已 adopt 则漂多少都不变)/ AI 规则接管时序号来自 AI 规则集且 config 里那档必须定不出序号 / 超 9 档退化为 0 而非溢出 / `-race` 并发一致 / 交易所回显伪造越界序号必须拒绝且不丢形状兜底 / 挂单后策略档位表被改必须靠交叉验证挡住误认 / 健康的两档账本一条都不能动 / 端到端闭环(ATR 规则 → tag → clientID → 解回序号 → 认领同一档,把 3 个包串起来跑;v1.17.1 正是三个环各自单测全绿而闭环不承载任何信息)/ reconcile 调用点集成(只测函数本身的话,把调用点删掉测试仍全绿)。
>
> **教训**:v1.17.1 的 13 例测试全部用**百分比单位**规则构造 fixture,而 5 个实盘策略里 4 个是 ATR 单位 —— 测试覆盖的是代码里那条**实盘根本不走**的分支。这不是"忘了测某个 case",而是 fixture 的默认选择与生产配置系统性错位。凡是"配置项会被运行时解析器改写"的功能,测试必须至少有一例走**解析后**的值,否则测的是另一个程序。
...
>
> **提交**:`6fd57e4`(6 个根因 + 25 个测试)。

> **🔥 v1.17.1 挂单自带档位标识 —— 多档 DD 的第二重身份(2026-07-29,提交 `8133f80`,未部署)**
>
> **为什么还需要第二重**:第一重身份是持久化的 `DynamicProtectionRecord.ExchangeOrderID`,三个 matcher(`hasMatchingNativeTrailingOrderForRule` / `findExistingFullTrailingOrder` / `findPartialTrailingReplacementCandidate`)与面板都已 ID-first,这部分**早已完成**(任务 #53–#56 复核确认)。但它有一个共同单点:**记录必须写成功且还在**。挂单成功而 persist 失败、交易所不回 orderID(Bitget 分支写空串)、记录被清,都会让这张单在交易所侧变成"无主 trailing 单",只能按 qty/激活价/回调率去猜属于哪一档 —— place-at-open 之后多档并存,这个猜法必然碰撞(v1.16.8 系列事故的根因)。
>
> **做法**:档位序号编进 `clOrdId`/`algoClOrdId`(交易所在挂单列表和成交里回显),格式 `<brokerPrefix><code2>["T"<档位1-9>]<nonce>`,仍 ≤32 字符。**`"T"` 作分隔符是关键**:nonce 是 hex,T 不可能出现在其中 → 解码无歧义,且旧 id(该偏移处不是 T)照旧能解出机制。
>
> - 档位序号 = 按 `MinProfitPct` 升序(次级键 `MaxDrawdownPct`/`CloseRatioPct`)在配置里的位次。**只用于兜底识别,绝不用于判定档位语义** —— 语义仍由 fingerprint 三元组定义,因为位次会随配置改动而变。
> - 档位走 **reason 字符串后缀**(`native_trailing#2`)而不是新增函数参数:reason 已经流经 10 个交易所适配器的 `Tagged*` 签名,而 `normalizeReason` 是所有机制比较的唯一收口点。在那里剥掉 `#N`,既有比较(OKX `cancelAlgoOrdersByReason` 的 `wantReason`、归因解码、`CodeForReason`)全部不动。**这条等价性必须有测试锁死**:一旦不等价,定向撤单会因"无注册机制码"整段跳过,或归因把 `native_trailing#2` 当未知机制而断链。
> - broker 前缀私有于各适配器包,所以解码放在 `okx/reason_codec.go` 与 `binance/reason_codec.go`,结果通过新字段 `OpenOrder.ProtectionTier` 传出来。
> - **顺手修掉 reclaim 的已知错认**:`drawdown_order_reclaim.go` 原本对 `activated` 单一律形状匹配为真,只能退回"取最低未用档",两档并存时会把 dd2 的单认给 dd1。现在改两级优先:先信单子自报的档位,旧单(`ProtectionTier=0`)才退回形状匹配。
>
> **反向验证**:禁用档位优先后 `TestReclaim_TieredOrderIsClaimedByItsOwnTierNotTheLowestUnused` 立刻红,实测把 dd2(close=100%)的单认成 dd1(close=30%),正是要修的错认。
>
> **测试** 13 例(store 6 / trader 7),含两条反向锁:① 旧单必须保留形状匹配认领能力 —— 否则线上现存挂单全变可撤,是**最坏退化**;② 两张单自报同一档时不得双重认领(第一张认领,第二张交回撤单名单)。
>
> **注意**:`trader/types/interface.go` 加字段会引起 struct tag 对齐漂移,该文件在 HEAD 是 gofmt 干净的,必须 `gofmt -w` 回去(仓库其它大量未格式化文件是基线,不要顺手动)。
