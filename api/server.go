...
func (s *Server) setupRoutes() {
	...
	api := s.router.Group("/api")
	{
		...
		s.route(api, "GET", "/ticker", "Real-time ticker price (?symbol=BTCUSDT&exchange=okx)", s.handleTicker)
...
Returns: {total_events, total_pnl, by_category:[{category,count,realized_pnl,fees}], by_mechanism:[{category,mechanism,count,realized_pnl,fees,close_value_usdt}]}`,
			...
			s.routeWithSchema(protected, "GET", "/positions/side-pnl-series", "Hourly realized PnL split by long/short for header sparkline",
				`Query: ?trader_id=<EXACT trader_id from GET /api/my-traders>&hours=<int, default 12, max 168>`,
				s.handleSidePnLSeries)
			s.routeWithSchema(protected, "GET", "/positions/breaker-history", "Recent circuit-breaker / breadth-guard close events",
				`Query: ?trader_id=<EXACT trader_id from GET /api/my-traders>&days=<int, default 7, max 90>&limit=<int, default 200, max 500>`,
				s.handleBreakerHistory)
...
Returns the most recent AI decision for each symbol analyzed in the last scan cycle.`,
			...
			s.routeWithSchema(protected, "GET", "/decisions/prompts", "Lazy-load a decision cycle's prompts",
				`Query: ?trader_id=<EXACT trader_id>&cycle=<int cycle_number>
Returns {system_prompt, input_prompt, cot_trace, raw_response} for one cycle. Kept out of the /decisions list payload so the list stays lightweight.`,
				s.handleDecisionPrompts)
